...
**What a plugin owes a page in return** — the whole bar is in [.agents/skills/biom-plugins/](.agents/skills/biom-plugins/SKILL.md) §3, and it is short: refuse in words where its node is misplaced, touch only the slot it was named, ink nothing, and tear down what it started.
...
**And a figure is a SCENE that PLAYS, on a loop.** Elements in the section's own markup, inked on the palette's tokens, running on one cycle behind the `--motion` switch so a reader who asked for stillness gets the resting frame: by default a scene that plays the passage through and comes round again, and a still figure only where nothing in the passage happens, moving even then in the one way that says what it is for. It is a visual and not a paragraph, with the fewest labels that make it readable. What makes one clear, and the brief an agent builds it from, is [.agents/skills/biom-animations/](.agents/skills/biom-animations/SKILL.md). **Its words come out of the section's `variables`**, so editing the document edits the picture. A raster pasted in is a picture OF a figure and takes neither the palette nor the width it is given; a figure that only sits there has spent the one thing it had over the sentence beside it; and a page of unbroken prose has drawn nothing at all. **A diagram of relationships is no exception** — [.agents/skills/biom-diagrams/](.agents/skills/biom-diagrams/SKILL.md). How a section holds a drawing and an animation without tripping the checker is [.agents/skills/biom-sections/](.agents/skills/biom-sections/SKILL.md).
...
**A passage is in exactly one of the two, and each links the other.** The line between them, and the test that settles where a new one goes, is [.agents/skills/biom-docs/](.agents/skills/biom-docs/SKILL.md).
...
**There is no model call inside the app.** The running workspace never asks you anything, so what you wrote is what somebody gets, cold, with no chance to explain it. What that requires of a page is in [.agents/skills/biom-pages/](.agents/skills/biom-pages/SKILL.md), and when a change actually reaches the screen is [docs/changes.md](docs/changes.md).
