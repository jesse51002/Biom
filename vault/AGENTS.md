...
**And a figure is DRAWN and MOVES: an inline `<svg>` in the section's own markup, animated with the page's own CSS and script.** A drawing is vector, so it is crisp at every width and takes the palette's tokens like everything else; a raster pasted in is a picture OF a figure and does neither. And a figure that arrives — the bars growing, the edge drawing itself, the row lighting when it is reached — reads before the prose does, which is the whole reason it is there. The motion is a reveal and never a loop for its own sake, and it rides on the `--motion` switch so a reader who asked for stillness gets it. **A diagram of relationships is no exception**: it is that same drawing with boxes and edges in it, laid out by the section's own script out of the nodes and edges in its `variables`, so editing the words edits the picture — no library and no diagram language shipped for anybody to learn — [.agents/skills/diagrams/](.agents/skills/diagrams/SKILL.md). How a section holds an `<svg>` and an animation without tripping the checker is [.agents/skills/sections/](.agents/skills/sections/SKILL.md).
