...
**R45 — the page's words are in `content.yaml`.** There are no `.md` files and no `.mermaid` files: a markdown part carries its prose inline, and a diagram is a drawing in a section's own markup laid out from the section's own variables. A file of prose beside `content.yaml` is invisible — nothing reads it, and two copies of a paragraph is one of them going stale. What a page's directory holds besides the document is **markup** — a section's own HTML file, `child.html`, and `_assets/` under it — **`markdown.yaml`**, this page's own type scale, and **`INSTRUCTIONS.md`**, the page's instructions to every run under it and every agent pointed at it (`docs/automations.md`): the one `.md` something reads, and not the page's words. *(FAIL)*
