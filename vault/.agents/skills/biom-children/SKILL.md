...
### Dressing or replacing the board

**The board is a plugin, `biom-holds`, and the document mounts it because its own variable says `foot: biom-holds`.** So the board is changed the way every framework plugin is changed — through its variables, in a rung — and never by copying the document:

- **Dress it**: name a plugin of your own at `head` — `head: board-look` in `plugins/biom-doc/extensions.yaml`, and a `plugins/board-look/` that appends a `<style>` keyed on `.g-holds`. That is how this workspace's own look sits on the framework's board and follows every release.
- **Replace it**: `foot: my-board` and a `plugins/my-board/` that reads `biom.children()` and draws the set its own way.
- **Drop it, for one page**: `foot:` left empty in that page's own `plugins/biom-doc/extensions.yaml`, and `rows: true` beside it if the bare reconciled rows should show instead.
- **Place it yourself**: a section says `<div data-g-plugin="biom-holds">` where the board should go — a gallery that wants it under its pictures — and the page's rung empties `foot` so it is not drawn twice.

The three rungs and how a value is refused are [`../../../docs/plugins.md`](../../../docs/plugins.md); the order to try things in, and why `doc` is never copied, is [`../biom-plugins/SKILL.md`](../biom-plugins/SKILL.md) §4.

**That is why nothing about a board is in `base/`.** A starter's look turns up in every page copied from it, and a board copied per folder page is the eighteen copies again. The argument that kept it out of `base/` was right; what changed is that there is now one line for it instead.
...
**Take it only where the board at the bottom is not the drawing this page wants** — a gallery of pictures, a grid of cards, a map. The board is still there underneath until the page says otherwise: `foot:` left empty in the page's own `plugins/biom-doc/extensions.yaml` is the one line that takes it away, and a section that wants the framework's board somewhere else places it with `data-g-plugin="biom-holds"` — §*Dressing or replacing the board*.
