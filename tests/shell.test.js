...
test("the bar holds Reload and nothing else, in both builds, on a page and off it", async () => {
  // THE BAR IS A BREADCRUMB AND THE FEW REAL ACTIONS. Reload is the one action
  // a page adds; Agent Terminal joins it when this window has a workspace to run
  // one in (tests/terminal-store.test.js and the dock tests cover that). There
  // is no menu, no Config, no History and no Modify page: the owner decided
  // (2026-09-17) that the three go rather than hide, so this is asserted in the
  // development build as well as the built one.
  for (const g of [harness(), harness(DOC, { view: "page", id: DOC.id }, true)]) {
    await tick();
    const tools = find(g.rail, (el) => has(el, "tools"));
    expect(findAll(tools, (el) => el.tagName === "BUTTON").map(flat)).toEqual(["Reload"]);
