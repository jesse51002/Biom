...
/* guest/plugins/grid.js — THE DOCUMENT'S OWN TABLE, DRAWN AS A BOARD AND EDITED
 * ONE CELL AT A TIME.
 *
 ...
 * one that draws it. It is not the `table` plugin: that one draws rows the
 * server holds in `workspace.db`, reached by name, edited in the workspace's
 * grid; this one draws rows the page holds, edited here, that travel with the
 * page and are projected into the mirror as a markdown table. Both look like a
 * board. Which to reach for is `docs/tables.md`.
 *
 ...
 * `currentColor` mixed down rather than a hex, for the reason `table.js` gives:
 * a box drawn before the theme lands must still be legible. The look is
 * deliberately plain — one hairline, the header in the furniture face, the
 * cells in the sheet face — because every workspace wears it until it edits
 * this file, which is its own copy.
 *
...
(function () {
  ...
  const SHEET = [
    ...
    "  max-width: 100%;",
    // THE WIDE THING SCROLLS, NOT THE PAGE — the rule `table.js` keeps and the
  ...
  glob.biom.plugins.register({
     ...
     *  `table`: a grid IS editable, cell by cell, and this file does the
