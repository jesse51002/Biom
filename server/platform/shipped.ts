...
/** Path under the directory asked about → every blob hash it has ever had. */
...
/** Every blob hash each file under `dir` has ever had in the history of the
 *  repository at `root`, keyed by path relative to `dir`.
 *
 ...
 *  of the old path, which is still in history under the old name. */
export async function shippedHashes(root: string, dir: string): Promise<Shipped> {
  const commits = await git(root, ["rev-list", "--all", "--", dir]);
  if (commits.code !== 0) return NOTHING_SHIPPED;
...
/** Is `text`, sitting at `rel`, a version the framework shipped at that path? */
export function isShipped(shipped: Shipped, rel: string, text: string): boolean {
  const had = shipped[rel];
